...
from eot.audio.pauses import frame_db, spans_from_quiet_mask
from eot.labeling import apptek
from eot.labeling.apptek import Decision, Segment, decide, merge_intervals, speech_intervals, words_in
...
def test_legacy_detector_is_the_quiet_mask_run_finder():
    """``silence_spans`` delegates to ``spans_from_quiet_mask`` (no trailing extension): identical output."""
    x, _ = _clip_with_pauses(np.random.default_rng(3), noise_db=-40.0)
    db = frame_db(x, SR, 20.0)
    quiet = db < max(db.max() - 35.0, -60.0)
    assert silence_spans(x, SR, min_silence=0.2) == spans_from_quiet_mask(quiet, 0.02, 0.2)
    assert silence_spans(np.zeros(0, np.float32), SR) == []
...
def test_heldout_accents_route_the_split_column(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(apptek, "load_wav", lambda path: np.zeros(5 * SR, dtype=np.float32))
    ...
    default, _, _ = apptek.label_conversation(conv, Detector(), grid=[0.2])
    routed, _, _ = apptek.label_conversation(conv, Detector(), grid=[0.2], heldout=frozenset({"en-AU"}))
    assert default[0].meta()["split"] == "train" and routed[0].meta()["split"] == "heldout"
