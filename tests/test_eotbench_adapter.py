...
def test_audio_only_onnx_adapter_omits_pruned_context(monkeypatch, tmp_path: Path) -> None:
    ...
    scores = adapter.predict_batch([{"audio": {"array": np.zeros(1600, dtype=np.float32), "sampling_rate": 16000}, "messages": []}])
    assert scores == [0.25]
...
def test_local_score_points_keep_original_span_indices():
    ...
    row = {
        ...
        "silence_spans": [{"start": 0.1, "end": 0.15}, {"start": 0.3, "end": 0.7}, {"start": 1.0, "end": 1.5}],
    }
    ...
    assert [r["silence_dur"] for r in points] == [0.2, 0.2]


def test_batched_predictions_preserves_order_and_flushes_the_remainder():
    ...
    class Adapter:
        def predict_batch(self, batch):
            ...
            return [float(item) for item in batch]

    items = ((i, {"id": i}) for i in range(5))
    rows = list(batched_predictions(Adapter(), items, batch_size=2))
    assert calls == [2, 2, 1]
    assert rows == [{"id": i, "p_eot": float(i)} for i in range(5)]
    assert list(batched_predictions(Adapter(), [], batch_size=2)) == []
