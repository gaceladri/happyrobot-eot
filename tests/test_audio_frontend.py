...
def test_vectorized_frontend_matches_whisper(length, normalize):
    x = np.random.default_rng(31).normal(0, 0.03, length).astype(np.float32)
    expected = _feature_extractor()(
        last_window(x), sampling_rate=16000, return_tensors="np", padding="max_length", truncation=True, do_normalize=normalize
    )["input_features"][0]
