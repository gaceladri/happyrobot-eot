...
def _rows(source: str, label: int, count: int) -> list[dict]:
    return [{"source": source, "clip_id": f"{source}-{index}", "label": label, "path": "unused.wav"} for index in range(count)]


def test_grouped_split_avoids_dominant_source_and_keeps_two_classes() -> None:
    rows = _rows("zero", 0, 10) + _rows("one", 1, 10)
    rows += [{"source": "dominant", "clip_id": f"dominant-{index}", "label": index % 2, "path": "unused.wav"} for index in range(80)]
...
def test_grouped_split_falls_back_to_clip_without_prefix_leakage() -> None:
    ...
    for label in (0, 1):
        for clip_index in range(3):
            ...
            rows.extend({"source": "only-source", "clip_id": clip_id, "label": label, "path": "unused.wav"} for _ in range(2))
