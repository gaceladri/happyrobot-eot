...
The selected source checkpoints and frozen validation panel were restored from the user's archive. See its README at `/media/ad/00b5422b-9d54-4449-8b5d-08eab5cdac8c/happyrobot-archive-2026-09-07/README.md`. The selected Whisper checkpoint hash is `d82bb9172b83251179b2dd759585532da4db72e38321160dd2b19542df406d4a`; the floating Cohere checkpoint hash is `98720ce3ee27a4298d47ae532dd06dcfc3893cd7928fff99c1213eff9ebe5ef4`.

```bash
.venv-deploy/bin/python -m research.labeling.experiments.L041.deployment.export_models whisper --out artifacts/deployment/whisper-cpu
.venv-deploy/bin/python -m research.labeling.experiments.L041.deployment.export_models cohere --out artifacts/deployment/cohere-gpu
.venv-deploy/bin/python -m research.labeling.experiments.L041.deployment.build_trt --onnx artifacts/deployment/cohere-gpu/eot.onnx --out artifacts/deployment/cohere-gpu/tuned.plan --precision fp16 --tuned
.venv-deploy/bin/python -m research.labeling.experiments.L041.deployment.verify_trt --engine artifacts/deployment/cohere-gpu/tuned.plan --source-dir artifacts/deployment/cohere-gpu --tolerance .02
```
