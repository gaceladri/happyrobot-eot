...
def make_turn(rng: random.Random, idx: int) -> dict:
    ...
    if cat == "mc_number":
        ...
        segs = [("we're in", 0.3), (rng.choice(CITIES), rng.choice([0.4, 0.7])), (rng.choice(["heading east", "empty since this morning", "about two hours out"]), 0.0)]
    elif cat == "availability":
        agent = "When can you pick up?"
        segs = [(rng.choice(["tomorrow", "Friday", "tonight"]), rng.choice([0.3, 0.6])), (rng.choice(["around", "probably"]), 0.4), (rng.choice(["eight a m", "noon", "six in the evening"]), 0.0)]
    elif cat == "short_answer":
        agent = rng.choice(["Is the truck a fifty three foot dry van?", "Can you do it for fifteen hundred?", "Anything else I can help with?"])
        ...
        segs = [(items[0], 0.5), (f"{items[1]}", 0.6), (f"and {items[2]}", 0.0)]
    return {"id": f"chal_{idx:04d}", "category": cat, "agent_text": agent, "segments": [{"text": t, "pause_s": p} for t, p in segs],
            "label": 1, "n_hold_decisions": sum(1 for _, p in segs if p > 0)}
