...
class EOTAdapter:
    ...
    def __init__(self, checkpoint: str | None = None, onnx: str | None = None, device: str | None = None):
        ...
        self.adapter_id = f"{self.adapter_id}-{'ctx' if self.use_context else 'audio'}-{self.model_sha[:12]}"
    ...
    def predict_batch(self, batch) -> list[float]:
        ...
        ctx = np.stack(
            [hash_context(last_assistant_text(item.get("messages"))) if self.use_context else np.zeros(CTX_LEN, np.int64) for item in batch]
        )
...
def batched_predictions(adapter, items: Iterable[tuple[dict, dict]], batch_size: int) -> Iterator[dict]:
    """Yield ``{**meta, "p_eot": p}`` for ``(payload, meta)`` pairs, calling ``adapter.predict_batch`` per ``batch_size`` payloads."""
    pending, meta = [], []

    def flush():
        for p, m in zip(adapter.predict_batch(pending), meta):
            yield {**m, "p_eot": float(p)}
        pending.clear()
        meta.clear()
...
def _score_points(adapter: EOTAdapter, rows, min_silence: float, grid_step: float) -> Iterator[tuple[dict, dict]]:
    """``(payload, meta)`` per grid point of every silence span >= ``min_silence`` (the last span is EOT)."""
    for row in rows:
        ...
        for k, s in enumerate(spans):
            ...
            while t <= end + 1e-9:
                timestamp = round(t, 6)
                payload = {"audio": x[: int(np.floor(timestamp * SAMPLE_RATE + 1e-6))], "messages": row.get("messages") or []}
                yield (
                    payload,
                    {"id": str(row["id"]), "span_index": k, "timestamp": timestamp, "silence_dur": round(t - start, 6), "label": label},
                )
                t += grid_step


def score_rows(adapter: EOTAdapter, rows, min_silence: float = 0.1, grid_step: float = GRID_STEP, batch_size: int = 16) -> Iterator[dict]:
    """Yield eot-bench-style prediction rows for dataset rows in the public schema."""
    return batched_predictions(adapter, _score_points(adapter, rows, min_silence, grid_step), batch_size)


def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--checkpoint", default=None, help="torch checkpoint (default: $EOT_CHECKPOINT)")
    ap.add_argument("--onnx", default=None, help="ONNX model (default: $EOT_ONNX); takes precedence over --checkpoint")
    ap.add_argument("--dataset", default="livekit/eot-bench-data", help="Hugging Face dataset in the eot-bench schema")
    ap.add_argument("--lang", default="en", help="dataset config name")
    ...
    ap.add_argument("--out", type=Path, required=True, help="prediction rows (.jsonl) for eot-sweep")
