...
from eot.modeling.train import evaluate
from eot.modeling.training_utils import atomic_torch_save, filter_time_to_onset
...
def soup(
    ...
    *,
    ...
    model = average_models(load_checkpoint(left), load_checkpoint(right), alpha)
    out.mkdir(parents=True, exist_ok=True)
    atomic_torch_save({"cfg": asdict(model.cfg), "state_dict": model.state_dict()}, out / "candidate.pt")

    rows, _ = filter_time_to_onset(read_samples(samples))  # the parents' filter, so the dev split is theirs
    _, dev, diagnostics = grouped_split(rows, dev_frac=dev_frac, seed=split_seed, return_diagnostics=True)
    loader = DataLoader(
        MinedDataset(dev, seed=split_seed + 1, normalize_audio=model.cfg.normalize_audio),
        batch_size=64,
        num_workers=workers,
        collate_fn=collate,
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    metrics = soup(
        args.left, args.right, args.samples, args.out, args.alpha, split_seed=args.split_seed, dev_frac=args.dev_frac, workers=args.workers
    )
