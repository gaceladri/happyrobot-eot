...
def _parity_inputs(model: EOTModel, validation_samples: Path | None, rng: np.random.Generator) -> tuple[list[Check], int]:
    ...
    checks: list[Check] = []
    for n in PARITY_BATCH_SIZES:
        features = rng.normal(size=(n, N_MELS, N_FRAMES)).astype(np.float32)
        context = rng.integers(0, 4096, size=(n, CTX_LEN), dtype=np.int64) if model.cfg.use_context else np.zeros((n, CTX_LEN), np.int64)
...
def _quantize_int8(fp32_path: Path, wrapper: ExportWrapper, checks: list[Check], tolerance: float, has_real_audio: bool) -> dict:
    ...
    try:
        ...
        result = {
            ...
            "parity": parity,
            "max_abs_diff_p_eot": parity["max_abs_diff_p_eot"],
            "accepted": accepted,
            "size_mb": q.stat().st_size / 1e6,
            "parity_tol": tolerance,
        }
        if accepted:
...
def export(
    ...
    model = load_checkpoint(checkpoint).cpu().eval()
    wrapper = ExportWrapper(model).eval()
    out.parent.mkdir(parents=True, exist_ok=True)
    torch.onnx.export(
        wrapper,
        (torch.randn(2, N_MELS, N_FRAMES), torch.zeros(2, CTX_LEN, dtype=torch.long)),
        str(out),
        opset_version=opset,
        dynamo=False,
        verbose=False,
    ...
    if int8:
        ...
        if info["int8"]["accepted"]:
            ...
            q.with_suffix(".json").write_text(
                json.dumps(
                    {
                        **info,
                        "onnx": str(q),
                        "sha256": info["int8"]["sha256"],
                        "size_mb": info["int8"]["size_mb"],
                        "parity": info["int8"]["parity"],
...
def main(argv: list[str] | None = None) -> None:
    ...
    info = export(
        args.checkpoint, args.out, args.opset, args.int8, args.parity_tol, args.int8_parity_tol, args.validation_samples, not args.no_fuse
    )
