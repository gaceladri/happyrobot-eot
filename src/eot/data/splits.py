...
def split_diagnostics(train: list[dict], dev: list[dict], *, group_key: str, requested_group_key: str | None = None) -> dict:
...
def grouped_split(
    ...
    *,
    ...
    group_label_counts = {name: Counter(int(row["label"]) for row in by_group[name]) for name in group_names}
    ...
    mixed = [name for name in group_names if set(group_label_counts[name]) >= {0, 1} and leaves_train_labels({name})]
    ...
    if group_key == requested_group_key and group_key != "clip_id" and achieved_dev_frac > max(2.0 * dev_frac, dev_frac + 0.10):
        ...
        clip_diagnostics["fallback_reason"] = f"source-grouped dev fraction {achieved_dev_frac:.4f} exceeded guardrail"
        if return_diagnostics:
            return clip_train, clip_dev, clip_diagnostics
        return clip_train, clip_dev
    diagnostics = split_diagnostics(train, dev, group_key=group_key, requested_group_key=requested_group_key)
