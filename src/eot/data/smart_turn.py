...
def load_smart_turn_clips(
    ...
    for i, row in enumerate(ds):
        ...
        yield Clip(
            ...
            audio=x,
            label=label,
            source=str(row.get("dataset", "unknown")),
            agent_text=str(row.get("agent_text", "") or ""),
        )
...
def _validate_resume_metadata(path: Path, expected: dict) -> None:
    ...
    mismatches = {key: (current.get(key), value) for key, value in expected.items() if current.get(key) != value}
    if mismatches:
    ...
    *,
    ...
    if resume and existing and not metadata_path.exists():
        for key, expected in provenance.items():
            ...
            if found != {expected}:
                raise ValueError(f"cannot prove resume provenance for {key}: found {sorted(map(str, found))}, requested {expected!r}")
    ...
    for clip in clips:
        ...
        if prior is not None:
            if int(prior["label"]) != label:
                raise ValueError(f"clip id {clip_id!r} changed label from {prior['label']} to {label}")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not result["completed"]:
        raise RuntimeError("dataset stream ended before both class quotas were met; resume with a larger --max-rows")
