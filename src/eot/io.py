...
def cached_sha256(path: Path, cache: Path) -> str:
    """``sha256_file`` memoised in a JSON table keyed by (path, size, mtime), so a multi-GB file is hashed once per host.

    A cache that cannot be written (read-only snapshot directory) is silently skipped.
    """
    path = Path(path)
    stat = path.stat()
    ...
    if key not in table:
        table[key] = sha256_file(path)
        try:
            atomic_write_json(cache, table)
        except OSError:
            pass
    return table[key]
