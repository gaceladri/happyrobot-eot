...
def silence_spans(
    ...
    return spans_from_quiet_mask(db < thr, frame_ms / 1000.0, min_silence)


SILERO_VAD_VERSION = "v6.2.1"
SILERO_VAD_COMMIT = "7e30209a3e901f9842f81b225f3e93d8199902b1"
SILERO_VAD_URL = f"https://raw.githubusercontent.com/snakers4/silero-vad/{SILERO_VAD_COMMIT}/src/silero_vad/data/silero_vad.onnx"
...
def spans_from_quiet_mask(quiet: np.ndarray, hop: float, min_silence: float, total: float | None = None) -> list[Span]:
    """Runs of ``True`` frames as spans of at least ``min_silence`` seconds; a trailing run extends to ``total`` if given."""
    spans: list[Span] = []
    i = 0
    n = len(quiet)
    while i < n:
...
class PauseSpan:
    ...
    @classmethod
    def trusted(cls, span: Span) -> PauseSpan:
        """A pause taken from a single energy detector at full confidence (no VAD to disagree)."""
        return cls(span.start, span.end, span.end, 1.0, "high", span, None, agree_from=span.start)
...
class PauseDetector:
    ...
    def __call__(self, x: np.ndarray, sr: int = SAMPLE_RATE) -> list[PauseSpan]:
        ...
        if self.detector == "legacy":
            return [PauseSpan.trusted(s) for s in silence_spans(x, sr, min_silence=self.min_silence)]
        energy = silence_spans_adaptive(x, sr, min_silence=self.min_silence)
        if self.detector == "energy":
            return [PauseSpan.trusted(s) for s in energy]
