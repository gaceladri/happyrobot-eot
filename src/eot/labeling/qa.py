...
Checks (thresholds pre-registered in the labeling plan, archived branch `archive/research`, output/LABELING_PIPELINE_PLAN.md §2.4):
...
def check_boundaries(apptek_dir: Path, apptek_root: Path, tolerances=(0.1, 0.2)) -> dict:
    ...
    return {
        ...
        "recall_turn_boundaries": {
            f"@{int(t * 1000)}ms": round(eot_boundary_matched[t] / max(1, eot_boundary_total), 4) for t in tolerances
        },
...
def check_pstn(clips_manifest: Path, detectors: dict[str, PauseDetector], n_clips: int = 300, snrs=(12, 20, 30), seed: int = 0) -> dict:
    ...
    out: dict = {
        ...
        "jaccard_median": {},
        "jaccard_mean": {},
        "pauses_per_clip_clean": {},
        "pauses_per_clip_noisy": {},
    }
...
def check_single_vs_dual(apptek_dir: Path, horizons=(1.0, 2.0, 3.0, 5.0, float("inf"))) -> dict:
    ...
    out["oracle_eot_agent_response_gap_s"] = _pct(
        [d["agent_onset"] - d["pause_start"] for d in decided if d["label"] == 1 and d["agent_onset"] is not None]
    )
    for T in horizons:
        ...
        key.append(
            {
                ...
                "vad_p_speech_last_160ms": round(p_tail, 3),
            }
        )
...
def check_distribution(samples_manifest: Path, clips_manifest: Path | None = None) -> dict:
    ...
    out = {
        "n_samples": len(rows),
        "labels": dict(labels),
        "hold_to_eot_ratio": round(labels[0] / max(1, labels[1]), 2),
        "kinds": dict(kinds),
        "cut_confidence": dict(Counter(str(r.get("cut_confidence")) for r in rows)),
...
def main(argv: list[str] | None = None) -> None:
    ...
    run.add_argument(
        "--skip", nargs="*", default=[], choices=["boundaries", "pstn", "single_vs_dual", "listening", "distribution", "krisp_tail"]
    )
    ...
    if args.mined:
        step(
            "distribution",
            lambda: {str(m): check_distribution(m, args.smart_turn_clips if "smart" in str(m) else None) for m in args.mined},
        )
    if args.apptek:
