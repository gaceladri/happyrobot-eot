...
from eot.labeling.samples import EOT_CUSTOMER_SILENT_S, MAX_HOLD_S
...
def label_pause(
    *,
    ...
    must visibly transfer, with ``EOT_CUSTOMER_SILENT_S`` of observed future to confirm it.
    """
    ...
    if next_own_onset is not None and (not math.isfinite(next_own_onset) or next_own_onset < cut_time or next_own_onset > observed_until):
        raise ValueError("cut after onset or onset beyond observed future")
    if boundary_tolerance < 0 or any(
        e.kind not in KNOWN or not e.speaker or not math.isfinite(e.start) or not math.isfinite(e.end) or not 0 <= e.start < e.end
        for e in events
    ):
    ...
    horizon = min(observed_until, next_own_onset if next_own_onset is not None else observed_until, pause_start + MAX_HOLD_S)
    ...
    if floor:
        ...
        if observed_until < first.start + EOT_CUSTOMER_SILENT_S:
            return Target(None, "censored_transfer_confirmation")
        if next_own_onset is not None and next_own_onset < first.start + EOT_CUSTOMER_SILENT_S:
            return Target(None, "collision")
        return Target(1, "annotated_floor_transfer")
    if next_own_onset is not None and next_own_onset - pause_start <= MAX_HOLD_S:
